...
class PoetryClientFactory(ClientFactory):
    ...
    def buildProtocol(self, address):
        ...
        基类Factory的实现buildProtocol过程是：
            安装（创建一个实例）我们设置在protocol变量上的Protocol类与在这个实例(此处即PoetryProtocol的实例）
            的factory属性上设置一个产生它的Factory的引用（此处即实例化PoetryProtocol的PoetryClientFactory）。
        :param address:
        :return:
        """
        p = self.protocol()
...
def poetry_main():
    ...
    [ reactor.connectTCP(host, port, factory) for host, port in addresses ]
