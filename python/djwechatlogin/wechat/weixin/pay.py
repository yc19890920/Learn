...
class WxPayConf_pub(object):
    ...
    APPID = settings.MP_WX_APPID
    #JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    APPSECRET = settings.MP_WX_APPSECRET
    #受理商ID，身份标识
    MCHID = settings.PAY_WX_ID
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    KEY = settings.PAY_WX_KEY

    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = settings.PAY_WX_NOTIFY_URL
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        """生成接口参数xml"""
        #检测必填参数
        if any(self.parameters[key] is None for key in ("out_trade_no", "body", "total_fee", "trade_type")):
        ...
        if self.parameters["trade_type"] == "NATIVE" and self.parameters["product_id"] is None:
            raise ValueError("NATIVE need product_id parameters")

        
        self.parameters["appid"] = WxPayConf_pub.APPID  #公众账号ID
        self.parameters["mch_id"] = WxPayConf_pub.MCHID  #商户号
        self.parameters["spbill_create_ip"] = "127.0.0.1"  #终端ip      
        self.parameters["notify_url"] = WxPayConf_pub.NOTIFY_URL  #公众账号ID
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        return  self.arrayToXml(self.parameters)
...
class Notify_pub(Wxpay_server_pub):
    ...
    def returnSuccessXml(self):
        self.setReturnParameter('return_code', 'SUCCESS')
        self.setReturnParameter('return_msg', 'OK')
        return self.returnXml()
...
def test2():
    n = UnifiedOrder_pub()
    n.setParameter("body", "aaaabb") #商品描述
    timeStamp = time.time()
    out_trade_no = "{0}{1}".format(WxPayConf_pub.APPID, int(timeStamp*100))
    n.setParameter("product_id", '10002') #总金额
    n.setParameter("out_trade_no", out_trade_no) #商户订单号
    n.setParameter("total_fee", '1') #总金额
    n.setParameter("notify_url", WxPayConf_pub.NOTIFY_URL) #通知地址
...
def test3():
    ...
    body = """
    <xml><appid><![CDATA[wx78d3f61147d5747c]]></appid>
<bank_type><![CDATA[CFT]]></bank_type>
<cash_fee><![CDATA[1]]></cash_fee>
<fee_type><![CDATA[CNY]]></fee_type>
<is_subscribe><![CDATA[Y]]></is_subscribe>
<mch_id><![CDATA[1418083302]]></mch_id>
<nonce_str><![CDATA[1j2crm2r8va0okjnyrnidw42ccvqnvaa]]></nonce_str>
<openid><![CDATA[oj8POwXAFJAUo18oEsW5bP-vKIdI]]></openid>
<out_trade_no><![CDATA[201612221445381K6JUYDFXXF5PTISAU]]></out_trade_no>
<result_code><![CDATA[SUCCESS]]></result_code>
<return_code><![CDATA[SUCCESS]]></return_code>
<sign><![CDATA[C58046F880D0C165CCDA936A4449173F]]></sign>
<time_end><![CDATA[20161222144604]]></time_end>
<total_fee>1</total_fee>
<trade_type><![CDATA[NATIVE]]></trade_type>
<transaction_id><![CDATA[4008882001201612223600663341]]></transaction_id>
</xml>
    """
    n = Notify_pub()
    n.saveData(body)
    ...
    print time_end
    import datetime
    print datetime.datetime.strptime(time_end, '%Y%m%d%H%M%S')
    print n.checkSign()
    print n.returnSuccessXml()
