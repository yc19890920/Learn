...
@register.filter
def is_text(field):
    return not (is_checkbox(field) or is_multiple_checkbox(field) or is_radio(field) or is_select(field))

# bool 字段展示成 按钮
@register.filter
def bootstrap_horizontal_bool(element, label_cols='col-sm-3 col-lg-3'):
    markup_classes = {'label': label_cols, 'value': '', 'single_value': ''}
    for cl in label_cols.split(' '):
        splitted_class = cl.split('-')
        try:
    ...
    return render_bool(element, markup_classes)

def render_bool(element, markup_classes):
    add_input_classes(element)
    template = get_template("bootstrapform/bool.html")
    context = {'field': element, 'classes': markup_classes, 'form': element.form}
    return template.render(context)


@register.filter
def bootstrap_flag(element):
    markup_classes = {'label': '', 'value': '', 'single_value': '', 'flag': 'true'}
    return render_flag(element, markup_classes)

def add_input_classes_flag(field):
    if not is_checkbox(field) and not is_multiple_checkbox(field) and not is_radio(field) \
        and not is_file(field):
        field_classes = field.field.widget.attrs.get('class', '')
        field_classes += ' form-control form-control-flag'
        field.field.widget.attrs['class'] = field_classes
    elif is_checkbox(field) or is_radio(field):
        field_classes = field.field.widget.attrs.get('class', '')
        field_classes += ' form-control-checkbox'
        field.field.widget.attrs['class'] = field_classes

def render_flag(element, markup_classes):
    element_type = element.__class__.__name__.lower()

    if element_type == 'boundfield':
        add_input_classes_flag(element)
        template = get_template("bootstrapform/field.html")
