...
KindEditor.plugin('localimage', function(K) {
    var self = this, name = 'localimage',
        allowImageUpload = K.undef(self.allowImageUpload, true),
        allowFileManager = K.undef(self.allowFileManager, false),
        //uploadJson = K.undef(self.uploadJson, self.basePath + 'php/upload_json.php'),
        uploadJson = K.undef(self.uploadJson, self.jsonUrl + '/template/ajax_upload_json/'),

        imgPath = self.basePath + 'plugins/image/images/',
        lang = self.lang('image' + '.');

    var imagelist = K.undef(self.imagelist, false);
    var tpl_id = K.undef(self.tpl_id, false);
    var user_id = K.undef(self.user_id, false);
    var img_type = K.undef(self.img_type, false);

    allowImageUpload = true;

    self.plugin.localImageDialog = function(options) {
        ...
        var option = '';
        for( i=0; i<length; i++ ) {
            option += '<option value="' + imagelist[i].filepath + '">' + imagelist[i].filename + '</option>';
        }
        ...
        var html = [
            ...
            '<input type="button" class="ke-button-common ke-button" name="viewServer" value="' + lang.upload + '" />',
            '</span>',
            '</div>',
        ...
        var uploadbutton = K.uploadbutton({
            ...
            fieldName : 'imgFile',
            width: 70,
            //url : K.addParam(uploadJson, 'dir=image&user_id='+user_id+'&tpl_id='+tpl_id+'&img_type='+img_type),
            url : K.addParam(uploadJson, 'dir=image&template_id='+tpl_id + '&user_id=' + user_id),
    ...
    self.plugin.localimage = {
        edit : function() {
            var img = self.plugin.getSelectedImage();
            self.plugin.localImageDialog({
    ...
    self.clickToolbar(name, self.plugin.localimage.edit);
});
